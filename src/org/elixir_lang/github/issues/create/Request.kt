...
class Request private constructor(val title: String, val body: String) {
    constructor(additionalInfo: String?, throwable: Throwable, attachmentList: List<Attachment>) : this(TITLE, body(additionalInfo, throwable, attachmentList))

    companion object {
        private const val TITLE = "[auto-generated]"

        private fun body(additionalInfo: String?, throwable: Throwable, attachmentList: List<Attachment>): String {
            val stringBuilder = StringBuilder()
            val level = 0
            version(stringBuilder)
            additionalInfo(stringBuilder, level + 1, additionalInfo)
            exception(stringBuilder, level + 1, throwable)
